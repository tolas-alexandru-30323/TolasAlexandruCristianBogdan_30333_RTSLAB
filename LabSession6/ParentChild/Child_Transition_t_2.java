...
public class Child_Transition_t_2 implements TransitionTemplate {
	...
	public Child_Transition_t_2(String name, PlaceHandlerTemplate PH, Integer delay) {
		this.Init(name, PH);
		this.SetDelay(delay);
	}

	public Child_Transition_t_2(String name, PlaceHandlerTemplate PH, Integer eet, Integer let) {
		this.Init(name, PH);
	...
	public Boolean TransitionGuardsMappings() {
		...
		if (!PH.GetPlaceByName("p_22").IsNull()) {
			
			toPrint=toPrint.concat(Print()+"\n");
			toPrint=toPrint.concat("ChildPH\n"+PH.PrintAllPlaces()+"\n");
			
			PH.GetPlaceByName("p_23").Set((Float)(PH.GetPlaceByName("p_22").Get())+ 0.1f);
			PH.GetPlaceByName("p_22").Set(null);
